<?php 
	if(isset($_GET["Search-box"])){
		$key = $_GET["Search-box"];
		$limit=4;
		$sql = "SELECT blog.`id`,blog.`nview`,blog.`avatar`,blog.`content`,blog.`dis`,user.`username`,typeblog.`nametypeblog`,blog.`timecreate`,blog.`title`,blog.`num` FROM blog,typeblog,user where blog.id_type_blog=typeblog.id AND blog.id_user=user.id AND (LOWER (blog.title) LIKE '%$key%' OR LOWER (blog.content) LIKE '%$key%' OR LOWER (typeblog.nametypeblog) LIKE '%$key%')  ORDER BY blog.timecreate DESC limit $limit";
		$getblog = mysqli_query($conn,$sql) or die("Lỗi select $sql");
		$selectnum="SELECT blog.`id`,blog.`nview`,blog.`avatar`,blog.`content`,blog.`dis`,user.`username`,typeblog.`nametypeblog`,blog.`timecreate`,blog.`title`,blog.`num` FROM blog,typeblog,user where blog.id_type_blog=typeblog.id AND blog.id_user=user.id AND (LOWER (blog.title) LIKE '%$key%' OR LOWER (blog.content ) LIKE '%$key%' OR LOWER (typeblog.nametypeblog) LIKE '%$key%')";
			...
			<div class="col-lg-8 post-list">
				...
				<div class=" latest-post-wrap">
		            ...
		            <ul class="pagination" style="margin-left: 234px;">
					  ...
					  ?>
					  <li class="page-item"><a <?php if($i==1)echo" style='background:#a5b8fb'" ?>  class="page-link pagecl2" href="javascript:void(0)"><?=$i?></a></li>
					  <?php } ?>
